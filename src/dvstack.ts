...
/**
 * Creates a vertical stack layout node with dynamic content support.
 *
 ...
 *
 * A dynamic vstack allows for it's children to be modified at runtime.
 *
 * @returns A Node-like API with layout configuration methods
 */

export const dvstack = (renderFunction: () => Node[]) => {
  let _hgap = 0;
  let _vgap = 0;
  let _center = false;
  ...
  const api = {
    ...
    _render(
      ...
      for (const child of renderFunction()) {
